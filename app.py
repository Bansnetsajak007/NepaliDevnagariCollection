...
if IS_CLOUD:
    # CLOUD MODE (Render): Save to 'nepali_dataset_cloud'
    BASE_DIR = 'nepali_dataset_cloud'
    print(f"[*] RUNNING ON CLOUD. Saving to: {BASE_DIR}")
else:
    # LOCAL MODE (Your Laptop): Save to Desktop
    desktop_path = os.path.join(os.path.expanduser("~"), "Desktop")
    BASE_DIR = os.path.join(desktop_path, "nepali_devnagaridataset")
    print(f"[*] RUNNING LOCALLY. Saving to: {BASE_DIR}")
...
def save_image():
    try:
        ...
        # FIX: Use BASE_DIR here (not DATASET_FOLDER)
...
def download_data():
    """
    Zips the BASE_DIR and lets you download it.
    """
    archive_name = "nepali_dataset_backup"
    
    # FIX: Ensure we are zipping the same folder we saved to (BASE_DIR)
    shutil.make_archive(archive_name, 'zip', BASE_DIR)
